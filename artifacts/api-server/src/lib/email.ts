    ...
    <tr><td align="center">
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#111111;border:1px solid rgba(255,255,255,0.08);border-radius:16px;overflow:hidden;">
        ...
        <tr><td style="background:#0a0a0a;padding:18px 32px;text-align:center;border-top:1px solid rgba(255,255,255,0.08);">
          <a href="${SHOP_WEBSITE}" style="color:#00E5FF;text-decoration:none;font-size:13px;">apexdetailing.net</a>
...
function detailsTable(rows: { label: string; value: string; highlight?: boolean }[]): string {
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:20px;margin-bottom:20px;">${rows
    .map(
      (r) =>
        `<tr><td style="padding:8px 0;color:#999999;font-size:13px;">${escapeHtml(r.label)}</td><td style="padding:8px 0;color:${r.highlight ? "#00E5FF" : "#ffffff"};font-size:${r.highlight ? "16px" : "14px"};font-weight:${r.highlight ? "700" : "600"};text-align:right;">${escapeHtml(r.value)}</td></tr>`,
    )
    .join("")}</table>`;
}

function ctaButton(href: string, label: string, gradient = "linear-gradient(90deg,#FF1AD8,#00E5FF)"): string {
...
export function customerHtml(b: BookingEmailData): string {
  ...
  const body = `
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:rgba(0,229,255,0.08);border:1px solid rgba(0,229,255,0.25);border-radius:12px;padding:18px;margin-bottom:24px;">
      <tr><td>
        <div style="color:#00E5FF;font-weight:700;font-size:14px;margin-bottom:6px;">Drop-Off Location</div>
    ...
    ${manageBlock}
    <p style="margin:0 0 8px 0;color:#cccccc;font-size:14px;line-height:1.6;">Prefer to talk to a human? Reply to this email or call us at <a href="tel:+14175276165" style="color:#00E5FF;text-decoration:none;font-weight:600;">${SHOP_PHONE}</a>.</p>
  ...
  return emailShell({
    ...
    headerGradient: "linear-gradient(135deg,#FF1AD8 0%,#00E5FF 100%)",
...
export function ownerHtml(b: BookingEmailData): string {
  ...
  return emailShell({
    ...
    headerGradient: "linear-gradient(135deg,#00E5FF 0%,#FF1AD8 100%)",
    eyebrow: "NEW BOOKING",
...
function customerCancelHtml(b: BookingEmailData, by: CancelledBy): string {
  const body = `
    ...
    <p style="margin:18px 0 0 0;color:#cccccc;font-size:14px;line-height:1.6;text-align:center;">Questions? Reply here or call <a href="tel:+14175276165" style="color:#00E5FF;text-decoration:none;font-weight:600;">${SHOP_PHONE}</a>.</p>
  `;
  return emailShell({
...
function customerRescheduleHtml(d: RescheduleEmailData): string {
  ...
  const body = `
    ...
    ${manageBlock}
    <p style="margin:18px 0 0 0;color:#cccccc;font-size:14px;line-height:1.6;text-align:center;">See you on ${escapeHtml(newStr)}!<br/>Questions? Reply here or call <a href="tel:+14175276165" style="color:#00E5FF;text-decoration:none;font-weight:600;">${SHOP_PHONE}</a>.</p>
  `;
