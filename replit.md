...
- **SMS** (`src/lib/sms.ts` + `src/lib/notify.ts` + `src/lib/reminders.ts`): Twilio-via-Replit-connector. Connector returns `{account_sid, api_key, api_key_secret, phone_number}`; client built with API-key auth and cached for 10 minutes. **`sendSms()` never throws** — all errors (invalid number, opt-out 21610, Twilio outage) are logged and swallowed so a failed text never blocks a booking. **`normalizeUsPhone()`** coerces "(417) 555-1234" / "417.555.1234" / "+14175551234" to E.164; non-coercible numbers skip the send.
  ...
  - **A2P 10DLC compliance**: every customer-facing template ends in "Reply STOP to opt out." Booking form (`pages/booking.tsx`) has an **optional** SMS consent checkbox labeled "Optional: it's ok to text me…" which writes the boolean into `bookings.sms_consent` (`smsConsent` in code). The Twilio campaign description promises "consent is not a condition of booking — leave this unchecked and we'll only contact you by email and phone", so `notify.ts` enforces this via a single `maySmsCustomer()` helper that gates every customer-facing `sendSms`. The `BookingEmailData` interface carries `smsConsent` so the reminder cron + reschedule/cancel paths see it too. Twilio handles STOP/HELP auto-responses; opted-out recipients return error code 21610 which we log+swallow.
